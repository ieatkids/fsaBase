...
MARKET_FIELDS = ['Time', 'Price', 'Volume', 'B1', 'B2', 'B3', 'B4', 'B5', 'A1', 'A2', 'A3', 'A4', 'A5', 'BQ1', 'BQ2', 'BQ3', 'BQ4', 'BQ5', 'AQ1', 'AQ2', 'AQ3', 'AQ4', 'AQ5']
...
def _getOrderAndTradeDfFromRawH5(exchange, instrument, date):  # 读取h5文件并返回order data和trade data
    ...
    orderDf['Time'] = orderDf['OrderTime'].str.decode('utf-8')
    tradeDf['Time'] = tradeDf['TradeTime'].str.decode('utf-8')
    orderDf['Time'] = orderDf['Time'].apply(lambda _: datetime.strptime(_, '%Y%m%d %H:%M:%S.%f'))
    tradeDf['Time'] = tradeDf['Time'].apply(lambda _: datetime.strptime(_, '%Y%m%d %H:%M:%S.%f'))
...
def _getMarketDfFromRawH5(exchange, instrument, date, freq=1):   # 从raw data获取market data，freq的单位是秒
    ...
    df2 = tradeDf.pivot_table(index='Index', values='Volume', aggfunc='sum')
